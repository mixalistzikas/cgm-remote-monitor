...
function init(ctx) {
  ...
  iob.calcTotal = function calcTotal(treatments, devicestatus, profile, time, spec_profile) {
    return addDisplay(iob.customIOB(treatments, profile, time, spec_profile));
  ...
  iob.customIOB = function calculateIOB(treatments, profile, time, spec_profile) {
    console.log('===============================');
    console.log(profile);
    console.log("spec_profile ", spec_profile);
    console.log("Date.now() ", Date.now());
    console.log("time ", time);
    let DIA_minutes = 120;
    if (profile !== undefined) {
      DIA_minutes = profile.getDIA(time, spec_profile) || 3;
      DIA_minutes = DIA_minutes * 60;
      console.log("DIA_minutes ", DIA_minutes);
    }
    
    const now = time;
  
    const grouped = treatments
      .filter(t => t.insulin > 0)
      .map(t => ({
        ...t,
        millsNew: Math.round(t.mills / 10000),
      }))
      .reduce((acc, curr) => {
        const key = `${curr.millsNew}-${curr.insulin}`;
        if (!acc[key]) acc[key] = [];
        acc[key].push(curr);
        return acc;
      }, {});
  
    const recentTreatments = Object.values(grouped)
      .map(group => group[0]) // παίρνουμε μόνο το πρώτο από κάθε group
      .filter(t => {
        const minutesAgo = (now - new Date(t.timestamp).getTime()) / 1000 / 60;
        return minutesAgo <= DIA_minutes;
      });
  
    const totalIOB = recentTreatments.reduce((sum, t) => {
      const minutesAgo = (now - new Date(t.timestamp).getTime()) / 1000 / 60;
      const ratio = Math.max(1 - (minutesAgo / DIA_minutes), 0);
      return sum + t.insulin * ratio;
    }, 0);
  
    return {
      treatments: recentTreatments,
      iob: Number(totalIOB.toFixed(2))
    };
  }
